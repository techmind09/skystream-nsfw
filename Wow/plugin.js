...
(function () {
    ...
    async function getHome(cb) {
        try {
            ...
            // Sabhi categories ko ek hi main dictionary me merge kiya gaya hai
            const allCategories = {
                ...
                "PAWG": `${baseUrl}/pawg/`,
                "Posing": `${baseUrl}/posing/`,
                "Petite": `${baseUrl}/petite/`,
                "Pick up": `${baseUrl}/pick-up/`,
                "Piercing": `${baseUrl}/piercing/`,
                ...
                "Valentine's day": `${baseUrl}/valentines-day/`,
                "Virgin": `${baseUrl}/virgin/`,
                "Vibrator": `${baseUrl}/vibrator/`,
                "Vietnamese": `${baseUrl}/vietnamese/`,
                "Vintage": `${baseUrl}/vintage/`,
                "Voyeur": `${baseUrl}/voyeur/`,
                "VR": `${baseUrl}/vr/`
            };
            
            const data = {};
            
            // Sabhi categories par loop chalega aur app ko process karke dynamic rows return karega
            for (const [categoryName, url] of Object.entries(allCategories)) {
                try {
                    const res = await http_get(url, HEADERS);
                    if (res.status === 200 && res.body) {
                        const items = parseVideoItems(res.body);
                        if (items.length > 0) {
                            // Har category ko object me key ki tarah push karenge taaki UI me render ho
            ...
            // Fallback: Agar upar se kuch bhi load nahi hua toh homepage parse karega
    ...
    async function search(query, cb) {
        try {
            const baseUrl = (typeof manifest !== 'undefined' && manifest.baseUrl) ? manifest.baseUrl : "https://www.wow.xxx";

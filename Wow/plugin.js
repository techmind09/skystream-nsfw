...
(function () {
    ...
    function parseVideoItems(html) {
        ...
        // Pattern 1: Standard div item container
    ...
    async function getHome(cb) {
        try {
            ...
            // Corrected and updated categories list
            const categories = {
                ...
                "Housewife": `${baseUrl}/housewife/`, // Fixed typo from pqahousewife
            ...
            // Fetch all categories in parallel so one failing won't block the rest
            const fetchPromises = categoryKeys.map(async (categoryName) => {
                ...
                try {
                    ...
                    if (res.status === 200 && res.body) {
                        ...
                        if (items.length > 0) {
                            return { categoryName, items: items.slice(0, 20) };
                        }
                    }
                } catch (e) {
            ...
            // Collect successful data entries
            for (const result of results) {
                if (result.status === 'fulfilled' && result.value) {
                    data[result.value.categoryName] = result.value.items;
                }
            }
            
            // Fallback: If no categories could load, default to main page
            if (Object.keys(data).length === 0) {
                console.warn("All categories failed. Trying fallback to baseUrl.");
                const res = await http_get(baseUrl, HEADERS);
                if (res.status === 200 && res.body) {
                    const latestItems = parseVideoItems(res.body).slice(0, 20);
            ...
            if (Object.keys(data).length === 0) {
                return cb({ success: false, errorCode: "PARSE_ERROR", message: "Failed to load any categories or layout content" });
            }

 ...
 * Features: Fixed Metadata Parsers, Heavy-Duty Multi-Pattern Stream Fallback
 */
...
(function () {
    ...
    /**
     * Full-Scale Aggressive Stream URL Extractor
     * Matches loose hyperlinked server setups, direct buttons, and inline media sources
     * @param {string} html - Page HTML content
     * @returns {Array} List of extracted target stream assets
     */
    function extractAgressiveStreams(html, pageUrl) {
        const results = [];
        let match;

        // 1. Target dynamic anchor buttons containing specific keywords or file structures
        const looseLinkPattern = /href=["'](https?:\/\/[^"']+)["']/gi;
        while ((match = looseLinkPattern.exec(html)) !== null) {
            const streamUrl = match[1];
            
            // Filter targets that resemble active multimedia pipelines or content servers
            if (streamUrl && !streamUrl.includes('adscore') && !streamUrl.includes('wp-content')) {
                if (streamUrl.includes('get_file') || streamUrl.includes('download') || streamUrl.includes('stream') || streamUrl.includes('player') || streamUrl.includes('server') || streamUrl.match(/\.(mp4|m3u8)/i)) {
                    results.push({
                        url: streamUrl,
                        quality: streamUrl.includes('1080') ? "Server (1080p Ultra)" : "Server (720p Premium)"
                    });
                }
            }
        }

        // 2. Direct Iframes extraction
        const iframePattern = /<iframe[^>]*src="([^"]+)"[^>]*>/gi;
        while ((match = iframePattern.exec(html)) !== null) {
            const iframeUrl = match[1];
            if (iframeUrl && !iframeUrl.includes("ads")) {
                results.push({ url: iframeUrl, quality: "Stream Server (Embed)" });
            }
        }

        // 3. Fallback to native video element sources
        const videoPattern = /<source[^>]*src="([^"]+)"[^>]*>/gi;
        while ((match = videoPattern.exec(html)) !== null) {
            if (match[1]) results.push({ url: match[1], quality: "Direct Video Pipeline" });
        }
    ...
    /**
     * Load video streams using strict Multi-Pattern matching and base64 formatting
     * @param {string} url - Video page URL
     * @param {Function} cb - Callback function
     */

...
(function () {
    ...
    function parseVideoItems(html) {
        ...
        const itemPattern = /<div class="video-item">[\s\S]*?<a href="(https:\/\/www\.xprimehub\.hair\/videos\/[^"]+)"[^>]*>[\s\S]*?<img[^>]*src="([^"]+)"[^>]*alt="([^"]+)"[\s\S]*?<\/div>/g;
        ...
        if (items.length === 0) {
            const simplePattern = /<a href="(https:\/\/www\.xprimehub\.hair\/videos\/[^"]+)"[^>]*>[\s\S]*?<img[^>]*src="([^"]+)"[^>]*alt="([^"]+)"/g;
    ...
    async function load(url, cb) {
        try {
            ...
            const title = titleMatch ? titleMatch[1].replace(/\s*-\s*XPRIMEHUB\.HAIR$/, '').trim() : "Unknown";
    ...
    function parseVideoStreams(html) {
        ...
        const sourcePattern = /<source\s+src=['"](https:\/\/www\.xprimehub\.hair\/get_file\/[^'"]+)['"][^>]*label=['"]([\w\d]+p?)['"][^>]*>/gi;

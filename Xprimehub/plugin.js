 ...
 * Features: Latest Updates, Categorized Navigation, Search, Absolute Intent Browser Redirection
 */
...
(function () {
    ...
    async function load(url, cb) {
        try {
            ...
            const episode = new Episode({
                name: "Redirect to Cloud Server Link",
                url: url,  
                season: 1,
                episode: 1,
                posterUrl: posterUrl
    ...
    /**
     * Parses the cloud gateway buttons shown in your screenshot
        ...
        while ((match = btnPattern.exec(html)) !== null) {
            ...
            if (serverUrl && (serverUrl.includes('drive') || serverUrl.includes('cloud') || serverUrl.includes('press') || serverUrl.includes('direct') || serverUrl.includes('link'))) {
                if (!serverUrl.includes('adscore') && !serverUrl.includes('wp-content')) {
                    ...
                    if (serverLabel.toLowerCase().includes("g-direct")) serverLabel = "⚡ G-Direct [Instant Link]";
                    else if (serverLabel.toLowerCase().includes("v-cloud")) serverLabel = "🔥 V-Cloud [Resumable Link]";
                    else if (serverLabel.toLowerCase().includes("filepress")) serverLabel = "📁 Filepress [G-Drive Link]";
        ...
        if (cloudStreams.length === 0) {
            ...
            while ((match = loosePattern.exec(html)) !== null) {
                ...
                if (targetUrl && (targetUrl.includes('download') || targetUrl.includes('get_file') || targetUrl.includes('stream'))) {
                    if (!targetUrl.includes('adscore') && !targetUrl.includes('wp-content')) {
    ...
    /**
     * Load video streams with strict StreamResult external routing configuration
     * @param {string} url - Video page URL
     * @param {Function} cb - Callback function
     */
        ...
        try {
            ...
            const streams = cloudServers.map(server => {
                return new StreamResult({
                    url: server.url,             // Raw external landing page URL
                    source: server.quality,
                    isHtml: false,               // Turn off HTML inline content extractor wrapper
                    useExternalBrowser: true,    // Force absolute system external app fallback execution
                    headers: { 
                        "Referer": url, 

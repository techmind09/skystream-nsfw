...
(function () {
    ...
    async function load(url, cb) {
        try {
            ...
            // XMoviesForYou video pages ke iframe sources (Streamtape, Mixdrop, Voe etc.) ko extract karna
            const streams = [];
            
            // Find all potential iframe URLs inside the single video post page
            ...
            while ((iframeMatch = iframePattern.exec(html)) !== null) {
                ...
                // Har iframe source ko extractor chain pe bhejien
                await loadExtractor(iframeUrl, streams);
            }

            // Agar directly video links regex se mil jaye page script me (Fallback)
            const generalMatch = html.match(/["'](http[^"']+\.(?:m3u8|mp4)[^"']*)["']/i);
            if (generalMatch && streams.length === 0) {
                streams.push(new StreamResult({ url: generalMatch[1], source: "Direct Server" }));
            }
            ...
            // SkyStream manifest structure ke anusaar data store script pass karein
    ...
    async function loadStreams(url, cb) {
        try {
            ...
            // Agar video page ka url direct pass ho rha ho load function ke bypass me
    ...
    // ==========================================
    // ADVANCED MULTI-SERVER EXTRACTOR PATTERN
    // ==========================================
    ...
    async function extractStreamtape(url, streams) {
        try {
            ...
            if (match) {
                ...
                streams.push(new StreamResult({ url: videoUrl, source: "Streamtape" }));
            }
    ...
    async function extractMixdrop(url, streams) {
        try {
            ...
            if (fileMatch) {
                ...
                streams.push(new StreamResult({ url: videoUrl, source: "Mixdrop" }));
            }
    ...
    async function extractDoodStream(url, streams) {
        try {
            ...
            if (passMatch) {
                ...
                streams.push(new StreamResult({ url: finalUrl, source: "DoodStream" }));
            }

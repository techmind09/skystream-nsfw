 ...
 * Features: Local Server (127.0.0.1) Streaming Integration
 */
...
(function () {
    ...
    /**
     * 6. LOAD STREAMS (LOCAL SERVER ROUTER)
     * Puraane magic proxies ko hata kar direct local client/app proxy server (127.0.0.1:8080) se link kiya gaya hai.
     */
    async function loadStreams(url, cb) {
        try {
            // Video slug ya ID nikalne ke liye url parse kar rahe hain
            const urlParts = url.split('/').filter(Boolean);
            const videoSlug = urlParts[urlParts.length - 1] || "default_video";
            
            const streams = [];

            // 720p Local Server Stream
            streams.push(new StreamResult({
                url: `http://127.0.0.1:8080/stream/720p/${videoSlug}.mp4`,
                source: "Local Server (720p)",
                headers: { 
                    "Referer": url, 
            ...
            // 480p Local Server Stream
            streams.push(new StreamResult({
                url: `http://127.0.0.1:8080/stream/480p/${videoSlug}.mp4`,
                source: "Local Server (480p)",
                headers: { 
                    "Referer": url, 
                    "User-Agent": HEADERS["User-Agent"] 
                }
